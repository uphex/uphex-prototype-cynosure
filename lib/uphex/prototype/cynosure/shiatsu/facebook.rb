        ...
        module Facebook
          ...
          class Client
            ...
            def page_posts_impressions_paid(since)
              Metric.new('page_posts_impressions_paid',[[:page_posts_impressions_paid],[:day]],
                         get_insight_data('page_posts_impressions_paid',since))
            end

            def page_impressions_paid(since)
              Metric.new('page_impressions_paid',[[:page_impressions_paid],[:day]],
                         get_insight_data('page_impressions_paid',since))
            end
            ...
            def post_impressions_paid
              Metric.new('post_impressions_paid',[[:post_impressions_paid],[:lifetime]],
                         {:payload => post_aggregated_insight(posts,'post_impressions_paid')})
            end

            def post_impressions_fan_paid
              Metric.new('post_impressions_fan_paid',[[:post_impressions_fan_paid],[:lifetime]],
                         {:payload => post_aggregated_insight(posts,'post_impressions_fan_paid')})
            end

            def post_video_complete_views_paid
              Metric.new('post_video_complete_views_paid',[[:post_video_complete_views_paid],[:lifetime]],
                         {:payload => post_aggregated_insight(videos,'post_video_complete_views_paid')})
            end

            def post_video_views_paid
              Metric.new('post_video_views_paid',[[:post_video_views_paid],[:lifetime]],
                         {:payload => post_aggregated_insight(videos,'post_video_views_paid')})
            end
          end
        end
      end
