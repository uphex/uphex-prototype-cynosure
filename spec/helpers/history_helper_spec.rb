    ...
    context 'no initial' do
      it 'gets no data' do
        Timecop.freeze(Time.utc(2014,10,20)){
          ...
          history_helper =
            Uphex::Prototype::Cynosure::Shiatsu::Helpers::HistoryHelper.new(DateTime.new(2014,10,18),nil,0,data)
          expect(history_helper.history).to match_array(expected)
        }
      end
    ...
    context 'with initial' do
      it 'gets no data' do
        Timecop.freeze(Time.utc(2014,10,20)){
          ...
          history_helper =
            Uphex::Prototype::Cynosure::Shiatsu::Helpers::HistoryHelper.new(DateTime.new(2014,10,18),nil,2,data)
          expect(history_helper.history).to match_array(expected)
        }
      end
      ...
      it 'gets some data before since' do
        Timecop.freeze(Time.utc(2014,10,20)){
          ...
          history_helper =
            Uphex::Prototype::Cynosure::Shiatsu::Helpers::HistoryHelper.new(DateTime.new(2014,10,18,12),nil,2,data)
          expect(history_helper.history).to match_array(expected)
        }
      end
  ...
  context 'with previous data points' do
    it 'gets no data' do
      Timecop.freeze(Time.utc(2014,10,20)){
        ...
        history_helper =
          Uphex::Prototype::Cynosure::Shiatsu::Helpers::HistoryHelper.new(DateTime.new(2014,10,18),last_known_value,2,data)
        expect(history_helper.history).to match_array(expected)
      }
